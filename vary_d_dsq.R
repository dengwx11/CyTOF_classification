...
gamma <- simulate_gamma(iteration, K, as.numeric(D))
A0 <- gamma$gamma
W <- simulate_w(as.numeric(D),K,A0, p.0 = p.0, q.0=q.0, p.neg1= p.neg1, q.neg1 = q.neg1, 
                big_w_mean=big_w_mean, big_tau_w = big_tau_w, 
                small_w_mean=small_w_mean, small_tau_w = small_tau_w)
AS <- simulate_AS(as.numeric(D),K,W,corr=corr)
label.output <- simulate_label(as.numeric(D),N,K,prob_k)
X <- simulate_x(as.numeric(D),N,W,label.output$label, mean_var_ratio = mean_var_ratio)
true.H <- label.output$H
rownames(X) <- c(1:as.numeric(D))
colnames(X) <- c(1:N)
...
rst <- run(X,rst.para$para$lambda1,rst.para$para$lambda2,rst.para$para$mu,rst.para$para$eta,
            AS,A0,as.numeric(D),K,N, epsilon = 10^(-3),fixed_loop=2000)
...
write.table(results, paste0('/home/bz234/project/Results/CyTOF/simulation/vary_d/s2', '_d_', D, '.txt'))