...
getLambda1 <- function(object = 1,lambda_object.sequence, theotherLambda, mu.init = 10,eta,epsilon = .1,fixed_loop=1500){
    rst.seq <- list()
    L1.seq <- c()
    k=1
    ...
    if(object==1){
        for(lambda_1 in lambda_object.sequence){
            ...
            rst.seq[[k]]<-run(X,lambda_1,theotherLambda,mu.init,eta,AS,A0,D,K,N, epsilon =  epsilon,fixed_loop=1500)
        ...
        for(lambda_2 in lambda_object.sequence){
            rst.seq[[k]]<-run(X,theotherLambda,lambda_2,mu.init,eta,AS,A0,D,K,N, epsilon =  epsilon,fixed_loop=1500)
            L1.tmp <- compute_L1(X,rst.seq[[k]])
            L1.seq <- c(L1.seq, L1.tmp)
...
getLambda2 <- function(object = 1,depth = 3,lambda_init, theotherLambda, mu = 10,eta,epsilon = .1,fixed_loop=1500){
    ...
    if(object == 1){
        rst.prev<-run(X,lambda.prev,theotherLambda,mu,eta,AS,A0,D,K,N, epsilon =  epsilon,fixed_loop=fixed_loop)
    }else{
        rst.prev<-run(X,theotherLambda,lambda.prev,mu,eta,AS,A0,D,K,N, epsilon =  epsilon,fixed_loop=fixed_loop)
    }
    ...
    for(layer in 1:depth){
        ...
        if(object == 1){
            rst.right <- run(X,lambda.right,theotherLambda,mu,eta,AS,A0,D,K,N,epsilon=epsilon,fixed_loop=fixed_loop)
            rst.left <- run(X,lambda.left,theotherLambda,mu,eta,AS,A0,D,K,N,epsilon=epsilon,fixed_loop=fixed_loop)  
        }else{
            rst.right <- run(X,theotherLambda,lambda.right,mu,eta,AS,A0,D,K,N,epsilon=epsilon,fixed_loop=fixed_loop)
            rst.left <- run(X,theotherLambda,lambda.left,mu,eta,AS,A0,D,K,N,epsilon=epsilon,fixed_loop=fixed_loop)
        }
...
getMu1 <- function(X, lambda_1,lambda_2,mu.sequence, eta,epsilon = 0.01,fixed_loop=500){
    rst.seq <- list()
    L1.seq <- c()
    sparsity.seq <- c()
    diff <- 10^8
    ...
    for(mu in mu.sequence){
        rst.seq[[k]]<-run(X,lambda_1,lambda_2,mu,eta,AS,A0,D,K,N, epsilon = epsilon,fixed_loop=fixed_loop)
...
getMu2 <- function(X, depth = 2, lambda_1,lambda_2,mu.init,eta,epsilon = .01,fixed_loop=1500){


    mu.prev <- mu.init
    global.max = mu.prev
    global.min = mu.prev
    rst.prev<-run(X,lambda_1,lambda_2,mu.prev,eta,AS,A0,D,K,N, epsilon =  epsilon,fixed_loop=fixed_loop)
    ...
    for(layer in 1:depth){
        ...
        rst.right <- run(X,lambda_1,lambda_2,mu.right,eta,AS,A0,D,K,N,epsilon=epsilon,fixed_loop=fixed_loop)
        rst.left <- run(X,lambda_1,lambda_2,mu.left,eta,AS,A0,D,K,N,epsilon=epsilon,fixed_loop=fixed_loop)  
